# Backstop â†’ Production: closing the gaps to the best depeg/crash insurance app on Sui

Status: execution. Owner: Backstop. Last updated: 2026-06-17.

Current status: duration-scaled premiums, max policy term, bounded governance
params, permissionless expired-policy cleanup, and the legacy RiskFeed-settled
`cover_pool` disable are implemented, unit-tested, and deployed through the
default mainnet `/depeg` pool.
...
| --- | --------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| G1  | Settles on a **single instantaneous** Pyth read â€” a transient wick or one manipulated update pays out                                 | `do_latch`: `assert!(price_mag <= threshold)` at one moment                                           | Critical â€” âœ… closed (dwell: two sub-threshold reads `min_dwell_secs` apart)                                                                                        |
| G2  | **Ignores the Pyth confidence interval** (`conf`)                                                                                       | `read_price_magnitude` reads only `get_price`, never `get_conf` (grep: no `conf`)                     | High â€” âœ… closed (PR #25: adverse-bound `price+conf<=threshold` + `max_conf_bps` reject)                                                                            |
| G3  | **Flat `premium_bps`** set at pool creation; no utilization curve, no cooldown â†’ adverse selection (buy cover at the moment of depeg) | `premium_for = cover * premium_bps / 10_000`                                                          | Critical (economic) â€” âœ… closed (cooldown via `activation_delay_secs`; utilization curve `rate = premium_bps + surge_premium_bps * (total_cover+cover)/pool_value`) |
| G4  | **No admin / pause / governance / timelock**; `premium_bps`/`threshold`/`max_age` frozen at creation                                    | grep: no `pause`/`AdminCap`/`owner`                                                                   | High â€” âœ… closed (`AdminCap` minted at creation; claim-exempt `set_paused`; timelocked propose/execute/cancel param updates)                                        |
| G5  | **No treasury fee** â€” 100% of premium to LPs, protocol not sustainable                                                                | â€”                                                                                                   | Medium â€” âœ… closed (`treasury_fee_bps` premium skim + admin-only treasury withdrawal)                                                                               |
| G6  | **No exposure caps** (per-policy / per-pool) on a fully-correlated single-feed risk                                                     | â€”                                                                                                   | High â€” âœ… closed (`max_cover_per_policy` + `max_total_cover` pool caps, 0 = uncapped; full collateralization kept)                                                  |
| G7  | **UpgradeCap** would sit in a hot EOA (the deployer)                                                                                    | `deployPackage.ts` transfers UpgradeCap to sender                                                     | High                                                                                                                                                                   |
| G8  | **No keeper incentive** to record a breach during the dip (Pyth is pull-based)                                                          | `record_breach` is permissionless but unrewarded                                                      | Medium â€” âœ… closed (`keeper_bounty` paid from treasury on confirm)                                                                                                  |
...
| G11 | **Risk terminal defaults to soonest/0d term** â†’ no curve on load; term options labeled by ambiguous `Nd`                              | `RiskTerminal.tsx:46` (idx 0); `predict.ts:202` soonest-first, no quoteability filter                 | Medium                                                                                                                                                                 |
| G12 | **App bundle ~632 kB** (no route-level code splitting)                                                                                  | single eager chunk in `App.tsx`                                                                       | Medium                                                                                                                                                                 |

(G10â€“G12 were surfaced by an independent Codex audit and verified against source this session.)
...
- **Use the confidence band, not the point price.** For a payout require the
  ...
  best-practice is to discount toward the adverse edge of the `price Â± conf` band
  (conf â‰ˆ 1Ïƒ). Also **reject** a read when `conf/price` exceeds a bound
  (`max_conf_bps`, e.g. 2%) â€” never settle while Pyth itself signals high
...
- **Sustained trigger (dwell), not a single read.** There is **no native on-chain
  TWAP on Sui** â€” must be built on top. Extend the latch into a dwell requirement:
...
- **Optional EMA cross-check** for large covers: also require
  ...
  floor. Reached via `price_info::get_price_feed` â†’ `price_feed::get_ema_price`; check
  its own timestamp for freshness (no `_no_older_than` wrapper exists).
- Keep `get_price_no_older_than`; tighten `max_age_secs` to ~30â€“60s for settlement.

### 1.2 Economics (closes G3, G5, G6)

- **Activation delay / cooldown.** âœ… **Done** â€” a freshly bought policy cannot record
  ...
  at the instant of depeg" â€” the single largest economic hole. (Mirrors Y2K's epoch
  pre-commitment.)
- **Utilization-based premium.** âœ… **Done** â€” `premium_for` now prices on a curve:
  ...
  scare and falls again as cover expires/claims free capacity â€” a state-driven
...
- **Exposure caps.** âœ… **Done** â€” pool fields `max_cover_per_policy` and
  `max_total_cover` (0 = uncapped), both enforced in `buy_cover`. **Full
  collateralization kept** â€” for a single-feed (fully correlated) depeg, fractional
  leverage is unsafe (every policy triggers at once); Nexus 2:1 leverage is **not**
  copied.
- **Treasury fee.** âœ… **Done** â€” `treasury_fee_bps` splits a protocol fee off each
...
- **Keeper bounty.** âœ… **Done** â€” `keeper_bounty` pays the confirming keeper from the
  treasury once when a breach latches; if treasury is empty, the latch still succeeds.

### 1.3 Safety & governance (closes G4, G7)

- **AdminCap + pause** â€” âœ… **Done**. `AdminCap` (carries `pool_id`) is minted to the
...
- **Timelocked parameter updates** â€” âœ… **Done**. `propose_param_update(kind, value)`
  ...
  `min_dwell_secs`, `activation_delay_secs`, and both caps â€” no silent live changes.
- **UpgradeCap â†’ multisig + timelock policy** (G7) in a _separate, immutable_ package; lock
  the policy right after publish; ratchet to `Additive`/`Dependency-only` once stable.
  (docs.sui.io/build/custom-upgrade-policy.)
- **Visibility sweep** â€” audit every `public` fn; keep internal helpers
  `public(package)`/private (the #1 real Sui exploit class â€” OpenZeppelin Sui bug
  patterns). Current helpers (`read_price_magnitude`, `do_latch`, `settle`)
  are already private â€” keep new ones so.

### 1.4 Keeper (closes G8)

- **Keeper bounty.** âœ… **Done** â€” a fixed `keeper_bounty` is paid from treasury to the
...
- **Buy cover** â€” connect wallet â†’ pick feed/threshold/cover/term â†’ one PTB
  (`updatePriceFeeds` + `buy_cover`) â†’ receive a `Policy` object. Live premium quote
  from the new utilization curve.
- **Provide liquidity** â€” `deposit_lp` / `withdraw_lp`, showing pool TVL, utilization,
  APR-from-premiums, and the full-collateralization headroom.
- **Settle a claim** â€” "Record breach" + "Claim" buttons that build the trustless
  one-PTB Pyth-update-then-settle flow; show dwell progress and payout.
- **My policies / My LP** â€” live positions, status, claimable state.
- **"Protect this position" rail** â€” local branch now scans wallet-owned
...
- **No-funds integration verifier** â€” `npm run verify:depeg` checks Pyth SUI/USD,
  ...
  funded wallet.
- **Dev tooling hardening** â€” the app is on Vite 8 / `@vitejs/plugin-react` 6 so the
...
- **Mainnet deployer key** â€” local Sui keystore alias `backstop-mainnet-deployer` was
...
- v1 insures **suiUSDe/USD** â€” the live mainnet Pyth `PriceInfoObject`
  (`0x9b2028â€¦d63f`, expo âˆ’8) was **verified readable this session**. Offer **USDe/USD**
...
- **Calibrate the threshold to Pyth's _aggregate_ behaviour**, not the worst
  ...
  ~$0.94â€“0.97 on aggregate; a Pyth-fed trigger tracks the aggregate. Backtest the feed
...
## 1.6 Merged execution plan (Backstop research Ã— Codex audit)
...
claim-exempt pause, timelock, multisig UpgradeCap) â€” independent convergence, high
confidence. It added three things this plan adopts: (a) real live-app **defects**
(G10â€“G12), (b) a **no-wallet simulator** as the bridge from read-only to interactive,
...
- **Sprint 0 â€” quick wins (now, no funds):** fix G10 (never overwrite a good feed
...
- **Sprint 1 â€” make `/depeg` the product (now, no funds):** no-wallet **simulator**
  (asset/coverage/floor/term â†’ premium, payoff, settlement path â†’ "Connect to
  execute") â†’ then wallet-connected mainnet **buy / LP / record-breach / claim** +
  "My policies / My LP", wiring the SDK `buildDepeg*` builders (devInspect-verified).
- **Sprint 2 â€” contract v2 hardening (now, no funds, parallel):** Â§1.1â€“1.4 â€” oracle
  hardening first (confidence band + dwell), then economics + governance, with full
  tests + Sui Prover. UI tracks the v2 API.
- **Sprint 3 â€” mainnet deploy + custody (done):** deployed v2, locked
  UpgradeCaps to dependency-only, transferred AdminCaps to custody, and proved
  live buy â†’ dwell â†’ claim through the staged mechanism-test pool.
- **Sprint 4 â€” assurance + polish:** audit, first real integration, proof-health
  badges, replayable accountability (decision â†’ blob â†’ on-chain reading â†’ outcome â†’
  Brier). The `/depeg` proof-health card is now buildable app code; audit and first
  real integration remain external.

The detailed protocol phases below (Â§2) are the engineering reference for Sprints 2â€“4.
...
### Phase 0 â€” Spec & calibration Â· _done_
...
### Phase 1 â€” Contract hardening (`pyth_cover_pool` v2) Â· _buildable now, no funds_
...
- Treasury fee split âœ… + keeper bounty âœ… (G5/G8 closed).
...
### Phase 2 â€” Interactive mainnet app Â· _buildable now; live txs need a funded wallet_
...
- Interactive flows: Buy cover, Provide/withdraw liquidity, Record-breach + Claim,
  My policies / My LP â€” all calling the SDK/app `buildDepeg*` builders (extended for
...
- Deploy step: published the hardened `pyth_cover_pool` and updated
  ...
  conservative capital, and updated `deployment.json`.
- Custody step: locked the new `UpgradeCap` policies to dependency-only and moved
  the new `AdminCap` objects to the custody wallet.
- Remaining verification beyond automation: one connected-wallet smoke against
  the replacement pool and one real NAVI/Suilend-style position-holder
  validation.
...
- Ship the "1 real integration" â€” a live policy against a real NAVI/Suilend suiUSDe
  position.
- **Acceptance:** audit report addressed; one external user/integration live;
  conservative caps in force.

### Phase 5 â€” Scale Â· _later_
...
| ---------------------------------- | ------------------------------- | ------------------------------------------------------------------------ |
| `threshold`                        | depeg floor                     | $0.985 @ expo âˆ’8 (= 98_500_000); calibrated on Oct-2025 USDe aggregate |
...
| `max_age_secs`                     | settlement freshness            | 30â€“60 s                                                                |
...
- **Oracle:** single-source Pyth. Mitigate with confidence band + dwell + freshness;
  ...
  flag on Sui â€” staleness is the liveness gate.
- **Economic:** correlated single-feed claims â†’ keep full collateralization + caps;
  never leverage. Adverse selection â†’ activation delay + utilization premium.
- **Centralization:** pause could censor payouts â†’ claim path pause-exempt; upgrade
  risk â†’ multisig + timelock + locked policy.
- **Smart-contract:** Move bug classes (visibility, type-param confusion, receipt-ID,
  rounding) â†’ visibility sweep, generic-`T` invariants (already `EWrongPool`/
...
**Recommended first build:** get one real protected position or protocol
integration. It is the highest-signal proof that Backstop is infrastructure
rather than a self-funded demo.
