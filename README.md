...
**Live on mainnet — Pyth-settled depeg cover:** DeepBook Predict is testnet-only with no committed mainnet date, so the production settlement path moves to **Pyth** (live on Sui mainnet). [`contracts/pyth_cover_pool`](./contracts/pyth_cover_pool) is a SUI-collateralized parametric depeg-cover pool whose claim reads a Pyth feed on-chain (`get_price_no_older_than` — freshness by construction) and pays iff the insured stablecoin breaks its floor. The cover pool passes **34/34** Move tests, the lending consumer passes **4/4**, the **suiUSDe feed is confirmed live on Sui mainnet**, and `deployment.json` records the live package IDs, dependency-only upgrade-policy locks, AdminCap custody transfer, and a staged buy → dwell → claim proof. The `app` "Depeg cover" tab includes wallet-connected buy/LP/record/claim flows, a live mainnet proof-health panel, and depeg helpers that point at the deployed mainnet pool by default.

**Readiness checks:** `npm run verify:readiness` runs app typecheck/build, agent and SDK typechecks, no-funds Pyth/NAVI/Suilend verification, no-wallet depeg PTB `devInspect`, custody checks, and public site/on-chain/Walrus proof checks. For browser layout smoke, run the app and then `npm --prefix app run smoke:depeg` (or set `DEPEG_SMOKE_URL=https://backstop.gudman.xyz/depeg`).
