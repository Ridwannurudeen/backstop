...
module risk_feed::risk_feed_tests {
    ...
    fun challenge_upheld_slashes_publisher() {
        ...
        // Uphold: Alice slashed 0.05 (→0.05 left), Bob paid 0.10 (bond + reward).
        ...
        assert!(coin::value(&paid) == 100_000_000, 3);
    ...
    fun challenge_rejected_forfeits_to_publisher() {
        ...
        sc.next_tx(BOB);
        risk_feed::challenge(&mut feed, key(), sui_coin(50_000_000, sc.ctx()), sc.ctx());
        // Reject: Bob's 0.05 joins Alice's stake → 0.15.
        risk_feed::resolve_challenge(&mut feed, &cap, key(), false, sc.ctx());
        assert!(risk_feed::stake_bond(&feed, ALICE) == 150_000_000, 0);
